...
__STATIC_INLINE int Chip_SPIM_HandlerRx(LPC_SPI_T *pSPI, SPIM_XFER_T *xfer)
{
	...
	if (xfer->rxCount > xfer->dataRXferred) {
		if (flen > 8) {
			((uint16_t *) xfer->rxBuff)[xfer->dataRXferred] = data;
		}
		else {
			((uint8_t *) xfer->rxBuff)[xfer->dataRXferred] = data;
		}
		ret = 1;
	}
	xfer->dataRXferred++;
	if ((xfer->rxCount == xfer->dataRXferred) && xfer->cbFunc) {
