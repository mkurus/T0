			...
			if(XModemReadByte(&u8_data)){
				...
				if(u32_byteCount == 0){
					...
					if(u8_data == SOH) {
						...
						for(i =0; i < sizeof(imageSign); i++)
							XModemReadByte(&imageSign[i]);

						u32_imageSize = MK_WORD32(imageSign[0], imageSign[1], imageSign[2], imageSign[3]);
						u32_imageCRC =  MK_WORD32(imageSign[4], imageSign[5], imageSign[6], imageSign[7]);

						PRINT_K("Writing signature...");
						WriteImageSignature(u32_imageSize, u32_imageCRC);
						SST25_WriteSR(0);  /*disable flash protection */
						return 0;
...
{
	SST25_WriteSR(0);
	SST25_WriteArray((uint8_t *)&u32_imageSize,
					  sizeof(u32_imageSize),
					  FIRMWARE_MAGICWORD_ADDRESS);

	SST25_WriteArray((uint8_t *)&u32_imageCRC,
					  sizeof(u32_imageCRC),
